var height, width, degree, active, unit, w_unit, theme, isActive = false, chart = [], graphData = ['', '', '', ''], chartID = [], startDate = [], chartColor = [], raw = ['', '', '', ''], timeout, today = new Date(), today = today.getDate() + '/' + (today.getMonth()+1), isInputActive = false, iconID, color;
			...
			if(i == 1) {
				...
				if(first == true) {
				    ...
				    img.onload = function() {
						...
						var iconData = {id: iconID, img: baseImg};
...
function fetching() {
	$.each(sounds, function(i) {
		...
		request.open("GET", "http://pattern.dk/sun/resources/" + sounds[i] + '.mp3', true);
	...
	if (!window.navigator.standalone) {
		...
		if(!window.navigator.onLine) {
			...
			if(localStorage.getItem("init2") != 'false') {
				...
				localStorage.setItem("city0-id", 'copenhagen+denmark');
				localStorage.setItem("city1-id", 'vienna+austria');
				localStorage.setItem("city2-id", 'hongkong+hongkong');	
			}
			...
			function check(input) {
				...
				if (name!=null && name!="") {
					$.ajax({ url:'http://maps.googleapis.com/maps/api/geocode/json?address=' + name + '&sensor=true&language=en',
							...
							if(data.status == 'OK') {
					    		...
					    		for (var i = 0; i < data.results[0].address_components.length; i++) {
					    			for (var j = 0; j < data.results[0].address_components[i].types.length; j++) {
					    		        if(data.results[0].address_components[i].types[j] == 'locality' && city == false) {
					    		            city = data.results[0].address_components[i].long_name;
					    		        }
					    		        if(data.results[0].address_components[i].types[j] == 'country' && country == false) {
					    		...
					    		var id = city.toLowerCase().replace(/[^a-zA-Z0-9]+/g, "")  + '+' + country.toLowerCase().replace(/[^a-zA-Z0-9]+/g, "");
					    		...
					    		$.ajax({
					    		  ...
					    		  success: function(data) {
					    		  	graphData[index + 1] = '';
					    		  	weather(data, index + 1, city);
					    		  	
					    		  	localStorage.setItem('city' + (index), city);
					    		  	localStorage.setItem('city' + (index) + '-lat', lat);
					    		  	localStorage.setItem('city' + (index) + '-lon', lon);
					    		  	localStorage.setItem('city' + (index) + '-id', id);
				    			...
				    			notification("The entered city didn't return any results.");
